...
MKDIR := mkdir -p
RM := rm -rf
CC := clang -Wno-unknown-warning-option -Wno-dangling-else -Wno-static-in-inline -O3
CFLAGS := -I$(EXTRA_INCLUDE) -c -o
LDFLAGS := -o
LDLIBS := -lm
#LDLIBS := -l"C:\Program Files\LLVM\lib\clang\17\lib\windows\clang_rt.builtins-x86_64.lib" -lkernel32.lib -ladvapi32.lib
...
ifeq ($(UNAME_S),Linux)
	CFLAGS := -fopenmp -fopenmp-version=50 $(CFLAGS)
	LDFLAGS := -fopenmp=libiomp5 $(LDFLAGS)
endif
