...
* целые числа и числа с плавающей точкой разной размерности:
  ...
  | `uint32` | Нет          | 32 бит     | 0..2^32-1       | `u32`   |
  | `int32`  | Да           | 32 бит     | -2^31..2^31-1   | `i32`   | Note that int32 and int are not converted one to another implicitly.
  | `uint64` | Нет          | 64 бит     | 0..2^64-1       | `u64`   |
  | `int64`  | Да           | 64 бит     | -2^63 .. 2^63-1 | `i64`   |
  | `half`   | Да           | 16 бит     |                 | `h`     | Currently, Ficus has no real support for such type, it's reserved for the future.
  ...
  |`int`     | Самый используемый тип. Это знаковый целый тип, совпадающий по размеру с указателем, то есть 32 бита на 32-битной машине, 64 бита на 64-битной машине и т.д. |
