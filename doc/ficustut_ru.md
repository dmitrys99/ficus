...
* целые числа и числа с плавающей точкой разной размерности:
  ...
  | Тип      | Знаковый тип | Размерность| Диапазон                                  | Суффикс | Примечание |
  |----------|--------------|------------|-------------------------------------------|---------|------------|
  | `uint8`  | Нет          | 8 бит      | 0&hellip;255                                    | `u8`    |
  | `int8`   | Да           | 8 бит      | -128&hellip;127                                 | `i8`    |
  | `uint16` | Нет          | 16 бит     | 0&hellip;65535                                  | `u16`   |
  | `int16`  | Да           | 16 бит     | -32768&hellip;32767                             | `i16`   |
  | `uint32` | Нет          | 32 бит     | 0&hellip;2<sup>32</sup>-1                       | `u32`   |
  | `int32`  | Да           | 32 бит     | -2<sup>31</sup>&hellip;2<sup>31</sup>-1         | `i32`   | Обратите внимание, `int32` и `int` друг в друга неявно не преобразуются.
  | `uint64` | Нет          | 64 бит     | 0&hellip;2<sup>64</sup>-1                       | `u64`   |
  | `int64`  | Да           | 64 бит     | -2<sup>63</sup>&hellip;2<sup>63</sup>-1         | `i64`   |
  | `half`   | Да           | 16 бит     | 2<sup>-14</sup>&hellip;65504                    | `h`     | В настоящий момент тип в Ficus не поддерживается, зарезервировано для будущих выпусков. 
  | `float`  | Да           | 32 бит     | -3.4e<sup>-38</sup>&hellip;3.4e<sup>+38</sup>   | `f`     |
  | `double` | Да           | 64 бит     | -1.7E<sup>-308</sup>&hellip;1.7E<sup>+308</sup> | &ndash; |
  | `long`   | Да           | &ndash;    | &ndash;                                         | &ndash; | Знаковый целый тип неограниченной (ограниченной потреблением памяти) точности.
  | `int`    | Да           | указатель  | &ndash;                                         | &ndash; | Самый используемый тип. Это знаковый целый тип, совпадающий по размеру с указателем, то есть 32 бита на 32-битной машине, 64 бита на 64-битной машине и т.д. |
