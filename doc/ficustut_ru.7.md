...
<tr>
	<td>`l.empty()`</td>
	<td>Возвращает истину, если список пуст.<br/>Можно также сравнить список с пустым списком: `l == []` или `l != []`, что транслируется в высокоэффективный код.</td>
</tr>
<tr>
	<td>`l.hd()`</td>
	<td>Извлекает первый элемент первой CONS-ячейки, то есть получает голову списка.<br/>Если список пуст, возникает исключение `NullListError`.</td>
</tr>
<tr>
	<td>`l.tl()`</td>
	<td>Извлекает второй элемент первой CONS-ячейки, то есть хвост списка.<br/>Если список пуст, возникает исключение `NullListError`.</td>
</tr>
<tr>
	<td>`x :: l`</td>
	<td>создаёт новый список путём добавления нового головного элемента `x` к существующему списку `l`</td>
</tr>
</table>
