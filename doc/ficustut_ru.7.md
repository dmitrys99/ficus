...
* `l.empty()` &ndash; возвращает истину, если список пуст. Можно также сравнить список с пустым списком: `l == []` или `l != []`, что транслируется в высокоэффективный код.
* `l.hd()` &ndash; извлекает первый элемент первой CONS-ячейки, то есть получает голову списка. Если список пуст, возникает исключение `NullListError`.
* `l.tl()` &ndash; извлекает второй элемент первой CONS-ячейки, то есть хвост списка. Если список пуст, возникает исключение `NullListError`.
* `x :: l` &ndash; создаёт новый список путём добавления нового головного элемента `x` к существующему списку `l`.
...
* `l.last()` &ndash; возвращает последний элемент списка; выбрасывает исключение `NullListError`, если список пуст.
* `l.nth(n)` &ndash; возвращает n-й элемент списка.
* `l.length()` &ndash; возвращает длину списка.
* `l.rev()` &ndash; возвращает обращённый список.
* `l.app(f)` &ndash; применяет функцию `f` ко всем элементам списка. Функция должна возвращать `void`.
* `l.map(f)` &ndash; создаёт новый список, применяя функцию `f` к каждому элементу.
* `l.foldl(f, v0)` &ndash; сворачивает список в значение, используя функцию обновления `f`. Начальное значение результата &ndash; `v0`.
* `l.array()` &ndash; преобразует список в массив.
* `l.all(pred)` &ndash; возвращает `true`, если предикат верен для всех элементов.
* `List.all2(la, lb, pred)` &ndash; аналогична `List.all()`, но применяется к двум спискам параллельно.
* `l.exists(pred)` &ndash; возвращает `true`, если предикат истинен хотя бы для одного элемента.
* `l.mem(a)` &ndash; возвращает `true`, если элемент `a` присутствует в списке.
* `l.find_opt(pred)` &ndash; возвращает первый элемент, для которого предикат верен. Если подходящего элемента нет, возвращается `None`.
* `l.assoc_opt(a)` &ndash; для списка пар `(aj, bj)` возвращает `Some(bj)` для первой пары, где `aj == a`. Если подходящей пары нет, возвращает `None`.
* `l.filter(pred)` &ndash; сохраняет только те элементы, для которых предикат верен.
* `ll.concat()` &ndash; объединяет несколько списков. Существует несколько вариантов этой функции. Для соединения двух списков достаточно записать `la + lb`.
* `List.zip(la, lb)` &ndash; возвращает список пар, состоящий из соответствий элементов списков `la` и `lb`.
* `lab.unzip()` &ndash; противоположна `List.zip()`, возвращает кортеж из двух списков (la, lb).
* `l.sort(lt)` &ndash; сортирует список с помощью функции сравнения `lt`:
...
* Дополнительные затраты памяти заметны только для маленьких коллекций. 
  Массивы требуют минимальных накладных расходов (0%), векторы &ndash; 
...
* чисто функциональная сортировка слиянием с временной сложностью 
  ...
  - выполняется `log⁡(N)` проходов по этому списку списков. На каждом проходе последовательно берутся 
    два соседних списка и сливаются в один. Итоговая сложность прохода &ndash; `O(N)`, 
